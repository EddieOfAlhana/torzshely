  ...
  return (
    <section id="about" className="relative py-24 md:py-32 bg-pub-dark overflow-hidden">
      <div className="absolute inset-0 opacity-5"
           style={{ backgroundImage: 'radial-gradient(circle, #5e909c 1px, transparent 1px)', backgroundSize: '40px 40px' }} />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid md:grid-cols-2 gap-16 items-center">
        {/* Image side */}
        <div ref={leftRef} className="relative">
          {/* Teal frame accent – behind image */}
          <div className="absolute -bottom-4 -right-4 w-32 h-32 border border-pub-teal/40" />
          <div className="absolute -top-4 -left-4 w-32 h-32 border border-pub-teal/20" />
          ...
          {/* Stats badge – gold accent */}
          <div className="absolute bottom-8 left-8 bg-pub-black/90 border border-pub-teal/30 px-6 py-4 backdrop-blur-sm">
            <div className="font-display text-3xl"><span className="text-pub-teal">4,9 </span><span className="text-pub-gold">★</span></div>
        ...
        {/* Text side */}
        <div ref={rightRef}>
          <div className="mb-2 text-pub-teal/60 font-display text-sm tracking-[0.4em] uppercase">
            Törzshely 16
          </div>
          ...
          <div className="space-y-5 text-pub-cream/75 leading-relaxed">
            <p>{t('about.p1')}</p>
            <p>{t('about.p2')}</p>
            <p className="text-pub-teal font-serif italic text-lg">{t('about.p3')}</p>
          </div>

          {/* Feature list */}
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {FEATURES.map(f => (
              <div key={f.key} className="flex items-start gap-3 p-4 border border-pub-teal/15 hover:border-pub-teal/45 transition-colors duration-300">
