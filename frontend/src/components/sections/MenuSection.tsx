    ...
    <section id="menu" className="py-24 md:py-32 bg-pub-black">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex justify-center mb-12">
          <div className="flex border border-pub-gold/30">
            <button onClick={() => setActiveTab('food')}
              className={`px-8 py-3 text-sm font-display tracking-widest uppercase transition-all duration-300 ${
                activeTab === 'food'
                  ? 'bg-pub-gold text-pub-black'
                  : 'text-pub-cream/60 hover:text-pub-gold'
              }`}>
              {t('menu.food')}
            </button>
...
function MenuCategoryBlock({ category, isHu }: { category: MenuCategory; isHu: boolean }) {
  ...
  return (
    <div ref={ref} className="border border-pub-gold/10 p-6 hover:border-pub-gold/30 transition-colors duration-500">
      <h3 className="font-display text-pub-gold text-sm tracking-[0.3em] uppercase mb-6 pb-3 border-b border-pub-gold/20">
        ...
        {category.items?.map(item => (
          <div key={item.id}
               className="group flex items-start justify-between gap-4 py-3 border-b border-pub-cream/5
                          hover:border-pub-gold/20 transition-colors duration-300">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="text-pub-cream font-medium group-hover:text-pub-gold transition-colors duration-300">
                  {isHu ? item.nameHu : (item.nameEn || item.nameHu)}
                </span>
                {item.featured && (
                  <span className="text-[10px] text-pub-gold border border-pub-gold/40 px-1.5 py-0.5 tracking-wider">★</span>
                )}
              </div>
            ...
            {item.priceHuf && (
              <div className="text-right flex-shrink-0">
                <span className="font-display text-pub-gold-light text-sm whitespace-nowrap">
